...
func (controller *DoctorController) GetProfile(c echo.Context) error {
	ctx := c.Request().Context()
	user, err := contextutil.GetUserFromContext(c)
	if err != nil {
		return response.Unauthorized(c, "Unauthorized")
	...
	filter := types.DoctorFilter{UserID: user.ID}
	doctor, err := controller.doctorSvc.Get(ctx, filter)
	if err != nil {
		return response.InternalServerError(c, "failed to get doctor profile")
	}

	return response.Success(c, "doctor profile fetched successfully", doctor)
}
